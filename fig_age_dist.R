...
age_distC = dcast(testC$dynamics[population %in% c("East of England", "South East", "London") & t %between% c(305, 340),
...
age_distU = dcast(testU$dynamics[population %in% c("East of England", "South East", "London") & t %between% c(305, 340),
    .(s1 = sum(Ip + Is + Ia), s2 = sum(Ip2 + Is2 + Ia2)), by = .(population, group, run)][,
    .(age = group * 5 - 2.5, s1 = s1 / sum(s1), s2 = s2 / sum(s2)), by = .(population, run)][,
        .(variant = rep(c("Preexisting", "VOC 202012/01"), each = 3), 
          value = c(quantile(s1, c(0.025, 0.5, 0.975)), quantile(s2, c(0.025, 0.5, 0.975))),
...
ggsave("./output/age_dist_model.pdf", width = 35, height = 14, units = "cm", useDingbats = FALSE)
ggsave("./output/age_dist_model.png", width = 35, height = 14, units = "cm")
