...
priorsI = list(
    ...
    season = "N 0 0.03 T 0 0.2",
    cfr_rlo2 = "N 0 0.1 T -2 2",
    icu_rlo2 = "N 0 0.1 T -2 2",
    cfr2_t = "N 0.5 0.1 T 0 1",
    sep_boost = "N 1 0.05",
    sep_when = "U 214 274"
)
...
for (replic in REP_START:REP_END)
{
    ...
    for (p in which_pops) {
        ...
        cm_source_backend(
            user_defined = list(
                model_v2 = list(
                    cpp_changes = cpp_chgI_baseline(priorsI),
                    cpp_loglikelihood = cpp_likI_baseline(paramsI, ldI, sitrepsI, seroI, virusI, sgtfI, p, "2020-11-15"),
    ...
    plot = check_fit(test, ld, sitreps, virus, sero, nhs_regions[which_pops], "2020-12-30")
    ggsave(paste0("./output/fit_baseline", replic, ".pdf"), plot, width = 30, height = 25, units = "cm", useDingbats = FALSE)
}
