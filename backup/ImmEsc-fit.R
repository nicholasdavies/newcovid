...
for (replic in REP_START:REP_END)
{
    ...
    for (p in which_pops) { ### 1:12) {
        ...
        qsave(rlang::duplicate(list(posteriorsI, parametersI)), paste0("./fits/ImmEsc-pp", replic, "-progress.qs"))
    ...
    qsave(rlang::duplicate(list(posteriorsI, parametersI)), paste0("./fits/ImmEsc-pp", replic, ".qs"))
    
    # Generate SPI-M output
    # Sample dynamics from fit
    # load_fit("./fits/ImmEsc-pp10.qs")
    dynamicsI = list()
    dynamics0 = list()
    for (p in which_pops)  { ### 1:12
    ...
    ggsave(paste0("./output/ImmEsc-variant_check_", replic, ".pdf"), plot1, width = 20, height = 6, units = "cm", useDingbats = FALSE)
    ...
    plot2 = ggplot(post) +
        ...
        expand_limits(x = 1)
    ggsave(paste0("./output/ImmEsc-variant_stats_", replic, ".pdf"), plot2, width = 20, height = 6, units = "cm", useDingbats = FALSE)
    
    qsave(plot1, "./output/cog-plot-ImmEsc.qs")
    qsave(plot2, "./output/post-plot-ImmEsc.qs")
    ...
    ggsave(paste0("./output/ImmEsc-fit_", replic, ".pdf"), plot3, width = 30, height = 25, units = "cm", useDingbats = FALSE)
}
